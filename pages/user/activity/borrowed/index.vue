                    ...
                    <div class="max-h-[80vh] overflow-y">
                        ...
                        <div
                            ...
                            <div class="mt-6 flex items-center gap-2" v-if="!isPreviewData">
                                ...
                                <label for="confirmData" class="text-sm text-gray-700 select-none">
    ...
    <div v-else class="overflow-x-auto rounded-lg bg-white">
        <table class="min-w-full text-sm text-left">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-else v-for="item in loanStore.loan" :key="item.id" class="hover:bg-gray-50">
                    <td class="px-4 py-3">
...
const formData = ref({
    ...
    teacherId: '',
    borrowerName: '',
    borrowDate: new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Jakarta' }).slice(0, 16),
    returnDate: new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Jakarta' }).slice(0, 16),
    purpose: '',
    room: '',
    guarantee: '',
    image: '',
...
const getModalTitle = () => {
    const titles = {
        ...
        'borrowable-student': isPreviewData.value ? 'Detail Borrowing' : 'Form Borrowing',
        'borrowable-teacher': isPreviewData.value ? 'Detail Borrowing' : 'Form Borrowing',
        ...
        if (response.ok) {
            ...
            if (data) {
                unitItem.value = {
                    ...
                    type: data.sub_item.item.name || data.unit_item.sub_item?.item?.name || '',
...
const openDetailModal = async (item) => {
    ...
    if (isConsumable) {
        currentModal.value = isStudent ? 'consumable-student' : 'consumable-teacher';
    } else {
        currentModal.value = isStudent ? 'borrowable-student' : 'borrowable-teacher';
    }
};
...
const openReturnModal = async (item) => {
    ...
    if (isConsumable) {
        currentModal.value = isStudent ? 'return-consumable-student' : 'return-consumable-teacher';
    } else {
        currentModal.value = isStudent ? 'return-borrowable-student' : 'return-borrowable-teacher';
    }
};
