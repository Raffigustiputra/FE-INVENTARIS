...
const route = useRoute(); // ⬅️ penting untuk ambil query
...
// ⬅️ ini bagian baru → cek query.message dari middleware
onMounted(() => {
  if (route.query.message) {
    showAlert("error", String(route.query.message));
  }
});
...
const login = async () => {
  try {
    ...
    if (response.data.role === "superadmin") {
      showAlert("success", "Login success as Superadmin!");
      router.push("/admin/dashboard");
    } else if (response.data.role === "admin") {
      showAlert("success", "Login success as Admin!");
      router.push("/kaprog/dashboard");
    } else if (response.data.role === "user") {
      showAlert("success", "Login success as User!");
  ...
  <div class="relative w-full h-screen flex items-center justify-center overflow-hidden">
    <div class="absolute right-8 top-10">
      <div class="flex items-center relative">
        <img src="/public/images/wv-blue.png" alt="Logo wv-blue" class="size-18 object-contain" />
    ...
    <div class="relative z-10 mx-24">
      ...
      <form @submit.prevent="login" class="w-5/12 pt-4 pb-8 rounded-xl bg-white">
        ...
        <div class="mx-4 mt-4 flex flex-col relati gap-1 relative">
          ...
          <input v-model="authStore.input.password" :type="typeInputPassword" placeholder="Input your password..."
          ...
          <button type="button" @click="switchVisibility"
            ...
            <svg v-if="switchEye === true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            ...
            <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="size-6 text-[#D5D5D5]">
              <path stroke-linecap="round" stroke-linejoin="round"
